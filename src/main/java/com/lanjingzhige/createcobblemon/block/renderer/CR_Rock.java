...
import com.lanjingzhige.createcobblemon.block.blockEntities.achieve.CBE_Rock;
...
public class CR_Rock extends KineticBlockEntityRenderer<CBE_Rock> {

    public CR_Rock(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    protected void renderSafe(CBE_Rock blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        // 让宝可梦面向方块 facing 方向
        Direction facing = blockEntity.getBlockState().getValue(CB_Rock.FACING);
    ...
    @Override
    public AABB getRenderBoundingBox(CBE_Rock blockEntity) {
        return new AABB(blockEntity.getBlockPos()).inflate(2.0);
    }

    @Override
    protected BlockState getRenderedBlockState(CBE_Rock be) {
        return be.getBlockState();
    }

    @Override
    protected SuperByteBuffer getRotatedModel(CBE_Rock be, BlockState state) {
