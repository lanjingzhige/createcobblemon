...
public class CR_Mulch extends KineticBlockEntityRenderer<CBE_Mulch> {

    public CR_Mulch(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    protected void renderSafe(CBE_Mulch blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        // 让宝可梦面向方块 facing 方向
        Direction facing = blockEntity.getBlockState().getValue(CB_Mulch.FACING);
    ...
    @Override
    public AABB getRenderBoundingBox(CBE_Mulch blockEntity) {
        return new AABB(blockEntity.getBlockPos()).inflate(2.0);
    }

    @Override
    protected BlockState getRenderedBlockState(CBE_Mulch be) {
        return be.getBlockState();
    }

    @Override
    protected SuperByteBuffer getRotatedModel(CBE_Mulch be, BlockState state) {
