...
import com.lanjingzhige.createcobblemon.block.blockEntities.achieve.CBE_Steel;
...
public class CR_Steel extends KineticBlockEntityRenderer<CBE_Steel> {

    public CR_Steel(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    protected void renderSafe(CBE_Steel blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        // 让宝可梦面向方块 facing 方向
        Direction facing = blockEntity.getBlockState().getValue(CB_Steel.FACING);
    ...
    @Override
    public AABB getRenderBoundingBox(CBE_Steel blockEntity) {
        return new AABB(blockEntity.getBlockPos()).inflate(2.0);
    }

    @Override
    protected BlockState getRenderedBlockState(CBE_Steel be) {
        return be.getBlockState();
    }

    @Override
    protected SuperByteBuffer getRotatedModel(CBE_Steel be, BlockState state) {
