...
public class CR_EvTrainer extends KineticBlockEntityRenderer<CBE_EvTrainer> {

    public CR_EvTrainer(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    protected void renderSafe(CBE_EvTrainer blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        // 让宝可梦面向方块 facing 方向
        Direction facing = blockEntity.getBlockState().getValue(CB_EvTrainer.FACING);
    ...
    @Override
    public AABB getRenderBoundingBox(CBE_EvTrainer blockEntity) {
        return new AABB(blockEntity.getBlockPos()).inflate(2.0);
    }

    @Override
    protected BlockState getRenderedBlockState(CBE_EvTrainer be) {
        return be.getBlockState();
    }

    @Override
    protected SuperByteBuffer getRotatedModel(CBE_EvTrainer be, BlockState state) {
